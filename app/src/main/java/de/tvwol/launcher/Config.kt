...
data class Config(
    ...
    fun resolvedCommand(): String =
        relayCommand.replace("%MAC%", normalizedMac ?: pcMac.trim())
    ...
    fun validate(): String? {
        ...
        if (useRelay) {
            ...
            if (relayCommand.isBlank()) return "Es ist kein Weck-Befehl eingetragen."
            if (relayCommand.contains("%MAC%") && normalizedMac == null) {
                return "Der Weck-Befehl enthält %MAC%, es ist aber keine gültige " +
                    "MAC-Adresse eingetragen."
            }
        }
        if (useDirectWol && normalizedMac == null) {
            return "Für Direkt-WoL wird eine gültige MAC-Adresse benötigt."
        }
